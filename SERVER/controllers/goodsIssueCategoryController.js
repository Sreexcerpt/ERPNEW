const GoodsIssueCategory = require('../models/GoodsIssueCategory');
...
const createGoodsIssueCategory = async (req, res) => {
  try {
    ...
    const category = new GoodsIssueCategory({ categoryName, prefix, rangeStart, rangeEnd });
    await category.save();
...
const getAllGoodsIssueCategories = async (req, res) => {
  try {
    const categories = await GoodsIssueCategory.find();
    res.json(categories);
  } catch (err) {
