...
exports.createVendorPrice = async (req, res) => {
  try {
    const { categoryId, vendorId, materialId, unit, bum, orderUnit,taxId,buyer } = req.body;

    if (!categoryId || !vendorId || !materialId || !unit || !bum || !orderUnit) {
      return res.status(400).json({ message: 'All fields are required' });
    }

    const newEntry = new VendorPriceList({ categoryId, vendorId, materialId, unit, bum, orderUnit,taxId,buyer });
    await newEntry.save();
...
exports.getAllVendorPrices = async (req, res) => {
  try {
    const entries = await VendorPriceList.find()
